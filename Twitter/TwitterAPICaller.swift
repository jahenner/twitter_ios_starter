...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    static let client = TwitterAPICaller(baseURL: URL(string: "https://api.twitter.com"), consumerKey: "u2oFgZDDkLQKN71d0dqdTdxzV", consumerSecret: "V9HQpYW5VAS76vzcGO3ca7LmQuOybS7VgQTeMbPqxz1yfFzgyX")
    var loginSuccess: (() -> ())?
    ...
    func retweet(retweetId: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let url = "https://api.twitter.com/1.1/statuses/retweet/\(retweetId).json"
        TwitterAPICaller.client?.post(url, parameters: ["id": retweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    func unretweet(retweetId: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let url = "https://api.twitter.com/1.1/statuses/unretweet/\(retweetId).json"
        TwitterAPICaller.client?.post(url, parameters: ["id": retweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
